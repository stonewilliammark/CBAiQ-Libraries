...
summary: "Added Pathway 3: Add to CBAiQ Libraries. Prompt builder now outputs README.md + version file pairs for the repo."
---
...
**Attach the CBAiQ Libraries Content Template** context file before starting. It contains the exact schemas and examples.
...
- For prompts: the prompt text, when to use it, any related context files or workflows
...
## Stage 3: Output README.md
Generate the README.md using the schema from the attached template. Fill all required front-matter fields. Write the prose body sections (use case, tips, etc.) in Australian English.

## Stage 4: Output version file
Generate the content file named `<prefix>_v1.0.md` (e.g., `prompt_v1.0.md`).
Include minimal front-matter: `date`, `author`, `summary`.
Body = the raw content (prompt text, workflow steps, context document, or solution description). No extra wrapper or metadata.

## Stage 5: File placement instructions
Tell the user:
1. Create folder: `content/<type>/<intent>/<slug>/`
2. Save the README.md into that folder
3. Save the version file into that folder
